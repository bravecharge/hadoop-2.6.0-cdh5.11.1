...
public class SchedulerApplicationAttempt {
	 ...
	 * application for a task at this priority, it is incremented, and each time the
	 * application successfully schedules a task, it is reset to 0.
	 */
	...
	public SchedulerApplicationAttempt(ApplicationAttemptId applicationAttemptId, String user, Queue queue,
		...
		if (rmContext.getRMApps() != null
				&& rmContext.getRMApps().containsKey(applicationAttemptId.getApplicationId())) {
			ApplicationSubmissionContext appSubmissionContext = rmContext.getRMApps()
					.get(applicationAttemptId.getApplicationId()).getApplicationSubmissionContext();
	...
	public synchronized RMContainer reserve(SchedulerNode node, Priority priority, RMContainer rmContainer,
			Container container) {
		...
		rmContainer.handle(
				new RMContainerReservedEvent(container.getId(), container.getResource(), node.getNodeID(), priority));
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("Application attempt " + getApplicationAttemptId() + " reserved container " + rmContainer
					+ " on node " + node + ". This attempt currently has " + reservedContainers.size()
					+ " reserved containers at priority " + priority + "; currentReservation "
	...
	public synchronized void showRequests() {
		if (LOG.isDebugEnabled()) {
			for (Priority priority : getPriorities()) {
				...
				if (requests != null) {
					LOG.debug("showRequests:" + " application=" + getApplicationId() + " headRoom=" + getHeadroom()
	...
	public synchronized void updateAMBlacklist(List<String> blacklistAdditions, List<String> blacklistRemovals) {
		if (!isStopped) {
			this.appSchedulingInfo.updateAMBlacklist(blacklistAdditions, blacklistRemovals);
		}
	}
	...
	/**
	 * Return the number of times the application has been given an opportunity to
	 * schedule a task at the given priority since the last time it successfully did
	 * so.
	 */
	 ...
	 * or when the scheduling locality threshold is relaxed. Reset various internal
	 * counters which affect delay scheduling
	 *
	...
	public synchronized ApplicationResourceUsageReport getResourceUsageReport() {
		...
		return ApplicationResourceUsageReport.newInstance(liveContainers.size(), reservedContainers.size(),
				Resources.clone(currentConsumption), Resources.clone(currentReservation),
	...
	public boolean equals(Object o) {
		...
		return (this == other || this.getApplicationAttemptId().equals(other.getApplicationAttemptId()));
	}
}
