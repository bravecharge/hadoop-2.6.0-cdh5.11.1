...
public class FSLeafQueue extends FSQueue {
	 ...
	 */
	private void updateStarvedAppsMinshare(final TreeSet<FSAppAttempt> appsWithDemand,
		...
		for (FSAppAttempt app : appsWithDemand) {
			if (!Resources.isNone(pending)) {
				...
				if (Resources.greaterThan(policy.getResourceCalculator(), scheduler.getClusterResource(), appMinShare,
						pending)) {
	...
	/**
	 * Helper method to identify starved applications. This needs to be called ONLY
	 * from {@link #updateInternal}, after the application shares are updated.
	 *
	 * A queue can be starving due to fairshare or minshare.
	 *
	 * Minshare is defined only on the queue and not the applications. Fairshare is
	 * defined for both the queue and the applications.
	 *
	 ...
	 *
	 * If this queue is starving due to fairshare, there must be at least one
	 * application that is starved. And, even if the queue is not starved due to
	 * fairshare, there might still be starved applications.
	 */
	...
	public Resource assignContainer(FSSchedulerNode node) {
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug(
					"Node " + node.getNodeName() + " offered to queue: " + getName() + " fairShare: " + getFairShare());
		}
...
//			assigned = sched.assignContainer(node); // yxd 原生
	 ...
	 * preemption-related code (as opposed to allocation), omits apps that should
	 * not be checked for starvation.
	 *
	 * @param assignment whether the apps are for allocation containers, as opposed
	 *                   to preemption calculations
	 * @return Set of apps with unmet demand
	 */
	private TreeSet<FSAppAttempt> fetchAppsWithDemand(boolean assignment) {
		TreeSet<FSAppAttempt> pendingForResourceApps = new TreeSet<>(policy.getComparator());
		readLock.lock();
		try {
			for (FSAppAttempt app : runnableApps) {
	...
	/**
	 * TODO: Based on how frequently this is called, we might want to club counting
	 * pending and active apps in the same method.
	 */
	 ...
	 * multiplying FairShare and maxAMShare. If FairShare is zero, use min(maxShare,
	 * available resource) instead to prevent zero value for maximum AM resource
	 * since it forbids any job running in the queue.
	 *
	...
	private Resource computeMaxAMResource() {
		...
		if (maxResource.getMemory() == 0) {
			maxResource
					.setMemory(Math.min(scheduler.getRootQueueMetrics().getAvailableMB(), getMaxShare().getMemory()));
		}

		if (maxResource.getVirtualCores() == 0) {
			maxResource.setVirtualCores(Math.min(scheduler.getRootQueueMetrics().getAvailableVirtualCores(),
	...
	/**
	 * Check whether this queue can run the Application Master under the maxAMShare
	 * limit.
	 *
	...
	private Resource minShareStarvation() {
		...
		Resource desiredShare = Resources.min(policy.getResourceCalculator(), scheduler.getClusterResource(),
				getMinShare(), getDemand());
	...
	protected void dumpStateInternal(StringBuilder sb) {
		ResourceWeights weights = scheduler.getAllocationConfiguration().getQueueWeight(getName());
		Resource maxShare = scheduler.getAllocationConfiguration().getMaxResources(getName());
		Resource minShare = scheduler.getAllocationConfiguration().getMinResources(getName());
		float maxAMShare = scheduler.getAllocationConfiguration().getQueueMaxAMShare(getName());

		sb.append("{Name: " + getName() + ", Weight: " + weights + ", Policy: " + policy.getName() + ", FairShare: "
				+ getFairShare() + ", SteadyFairShare: " + getSteadyFairShare() + ", MaxShare: " + maxShare
				+ ", MinShare: " + minShare + ", ResourceUsage: " + getResourceUsage() + ", Demand: " + getDemand()
				+ ", Runnable: " + getNumRunnableApps() + ", NumPendingApps: " + getNumPendingApps() + ", NonRunnable: "
				+ getNumNonRunnableApps() + ", MaxAMShare: " + maxAMShare + ", MaxAMResource: " + computeMaxAMResource()
				+ ", AMResourceUsage: " + getAmResourceUsage() + ", LastTimeAtMinShare: " + lastTimeAtMinShare + "}");
	}
}
