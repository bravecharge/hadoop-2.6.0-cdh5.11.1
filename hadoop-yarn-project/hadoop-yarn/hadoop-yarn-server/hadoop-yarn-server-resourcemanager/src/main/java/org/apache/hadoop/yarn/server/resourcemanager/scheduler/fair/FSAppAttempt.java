...
import org.apache.hadoop.yarn.server.resourcemanager.scheduler.fair.balancer.AllocateBalancer;
import org.apache.hadoop.yarn.server.resourcemanager.scheduler.fair.balancer.BalancerType;
...
public class FSAppAttempt extends SchedulerApplicationAttempt implements Schedulable {
	 ...
	 * containers over rack-local or off-switch containers. To acheive this we first
	 * only allow node-local assigments for a given prioirty level, then relax the
	 * locality threshold once we've had a long enough period without succesfully
	 * scheduling. We measure both the number of "missed" scheduling opportunities
	 * since the last container was scheduled at the current allowed level and the
	 * time since the last container was scheduled. Currently we use only the
	 * former.
	 */
	...
	synchronized void containerCompleted(RMContainer rmContainer, ContainerStatus containerStatus,
	...
	private synchronized void unreserveInternal(Priority priority, FSSchedulerNode node) {
		...
		LOG.info("Application " + getApplicationId() + " unreserved " + " on node " + node + ", currently has "
				+ reservedContainers.size() + " at priority " + priority + "; currentReservation "
				+ currentReservation);
	}
	...
	private void subtractResourcesOnBlacklistedNodes(Resource availableResources) {
		...
		for (FSSchedulerNode node : blacklistNodeIds) {
			Resources.subtractFromNonNegative(availableResources, node.getAvailableResource());
		}
	}

	/**
	 * Headroom depends on resources in the cluster, current usage of the queue,
	 * queue's fair-share and queue's max-resources.
	 */
	@Override
	public Resource getHeadroom() {
		...
		Resource maxAvailableResource = Resources.componentwiseMin(clusterAvailableResources,
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("Headroom calculation for " + this.getName() + ":" + "Min(" + "(queueFairShare=" + queueFairShare
					+ " - queueUsage=" + queueUsage + ")," + " maxAvailableResource=" + maxAvailableResource
					+ "Headroom=" + headroom);
		}
		return headroom;
	}
	 ...
	 * current size of the cluster and thresholds indicating how many nodes to fail
	 * at (as a fraction of cluster size) before relaxing scheduling constraints.
	 */
	...
	public synchronized NodeType getAllowedLocalityLevelByTime(Priority priority, long nodeLocalityDelayMs,
		...
		if (!allowedLocalityLevel.containsKey(priority)) {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("Init the lastScheduledContainer time, priority: " + priority + ", time: " + currentTimeMs);
			}
			allowedLocalityLevel.put(priority, NodeType.NODE_LOCAL);
			return NodeType.NODE_LOCAL;
		}
	...
	synchronized public RMContainer allocate(NodeType type, FSSchedulerNode node, Priority priority,
			ResourceRequest request, Container reservedContainer) {
		...
		if (container == null) {
			container = createContainer(node, request.getCapability(), request.getPriority());
		}
		...
		List<ResourceRequest> resourceRequestList = appSchedulingInfo.allocate(type, node, priority, request,
				container);
	...
	/**
	 * Should be called when the scheduler assigns a container at a higher degree of
	 * locality than the current threshold. Reset the allowed locality level to a
	 * higher degree of locality.
	 */
	...
	/**
	 * Reset the minshare starvation attributed to this application. To be called
	 * only from {@link FSLeafQueue#updateStarvedApps}
	 */
	...
	boolean canContainerBePreempted(RMContainer container) {
		...
		if (!getLiveContainersMap().containsKey(container.getContainerId())
			...
			LOG.error("Looking to preempt container " + container + ". Container does not belong to app "
					+ getApplicationId());
			return false;
		}
		...
		for (FSQueue q = getQueue(); !q.getQueueName().equals("root"); q = q.getParent()) {
			if (!q.isPreemptable()) {
				return false;
			}
		}
		...
		Resource usageAfterPreemption = Resources.subtract(getResourceUsage(), container.getAllocatedResource());
	 ...
	 * application on the given node with the given capability and priority.
	 */
	private Container createContainer(FSSchedulerNode node, Resource capability, Priority priority) {
		...
		return BuilderUtils.newContainer(containerId, nodeId, node.getRMNode().getHttpAddress(), capability, priority,
				null);
	}
	 ...
	 * This dispatches ro relevant handlers in {@link FSSchedulerNode}.. return
	 * whether reservation was possible with the current threshold limits
	 */
	private boolean reserve(ResourceRequest request, FSSchedulerNode node, Container reservedContainer, NodeType type) {
		...
		boolean reservableForThisApp = nodeReservedContainer == null
				|| nodeReservedContainer.getApplicationAttemptId().equals(getApplicationAttemptId());
	...
	private boolean reservationExceedsThreshold(FSSchedulerNode node, NodeType type) {
		...
		if (type != NodeType.NODE_LOCAL) {
			...
			int totalAvailNodes = (type == NodeType.OFF_SWITCH) ? scheduler.getNumClusterNodes()
			...
			if (existingReservations >= numAllowedReservations) {
				...
				if (LOG.isDebugEnabled()) {
					LOG.debug("Reservation Exceeds Allowed number of nodes:" + " app_id=" + getApplicationId()
							+ " existingReservations=" + existingReservations + " totalAvailableNodes="
							+ totalAvailNodes + " reservableNodesRatio="
							+ df.format(scheduler.getReservableNodesRatio()) + " numAllowedReservations="
	...
	int getNumReservations(String rackName, boolean isAny) {
		...
		if (isAny) {
			...
			Set<String> nodes = reservations.get(rackName == null ? "NULL" : rackName);
			if (nodes != null) {
				counter += nodes.size();
			}
		}
	 ...
	 * not have enough memory, create a reservation. This is called once we are sure
	 * the particular request should be facilitated by this node.
	 *
	 ...
	 * @param reserved Whether there's already a container reserved for this app on
	 *                 the node.
	...
	private Resource assignContainer(FSSchedulerNode node, ResourceRequest request, NodeType type, boolean reserved) {
		...
		if (isReservable(capability) && reserve(request, node, reservedContainer, type)) {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("Couldn't create reservation for app:  " + getName() + ", at priority "
						+ request.getPriority());
			}
			return Resources.none();
		}
	...
	private boolean isReservable(Resource capacity) {
		...
		return isStarved()
				&& scheduler.isAtLeastReservationThreshold(getQueue().getPolicy().getResourceCalculator(), capacity);
	}
	...
	private boolean isOverAMShareLimit() {
		...
		if (!isAmRunning() && !getUnmanagedAM()) {
			...
			if (ask.isEmpty() || !getQueue().canRunAppAM(ask.get(0).getCapability())) {
				return true;
			}
		}
		return false;
		...
		synchronized (this) {
			for (Priority priority : prioritiesToTry) {
				...
				if (offSwitchRequest != null && offSwitchRequest.getNumContainers() != 0) {
					if (!hasNodeOrRackLocalRequests(priority) || allowedLocality.equals(NodeType.OFF_SWITCH)) {
	...
	/**
	 * Whether this app has containers requests that could be satisfied on the given
	 * node, if the node had full space.
	 */
	...
	/**
	 * Called when this application already has an existing reservation on the given
	 * node. Sees whether we can turn the reservation into an allocation. Also
	 * checks whether the application needs the reservation anymore, and releases it
	 * if not.
	 *
	...
	boolean assignReservedContainer(FSSchedulerNode node) {
		...
		if (!isValidReservation(node)) {
			...
			LOG.info("Releasing reservation that cannot be satisfied for " + "application " + getApplicationAttemptId()
					+ " on node " + node);
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug(
					"Trying to fulfill reservation for application " + getApplicationAttemptId() + " on node: " + node);
		}
	...
	private boolean isUsageBelowShare(Resource usage, Resource share) {
		return fsQueue.getPolicy().getResourceCalculator().compare(scheduler.getClusterResource(), usage, share,
				true) < 0;
	}
	...
	/**
	 * Fetch a list of RRs corresponding to the extent the app is starved (fairshare
	 * and minshare). This method considers the number of containers in a RR and
	 * also only one locality-level (the first encountered resourceName).
	 *
	...
	List<ResourceRequest> getStarvedResourceRequests() {
		...
		for (ResourceRequest rr : appSchedulingInfo.getAllResourceRequests()) {
			...
			int numContainersThatFit = (int) Math
					.floor(Resources.ratio(scheduler.getResourceCalculator(), pending, rr.getCapability()));
			...
			if (numContainersThatFit < rr.getNumContainers()) {
				rr = ResourceRequest.newInstance(rr.getPriority(), rr.getResourceName(), rr.getCapability(),
	 ...
	 * outstanding demand. The app should not be considered starved until after the
	 * specified delay.
	 *
	 * @param delayBeforeNextStarvationCheck duration to wait
	 */
	void preemptionTriggered(long delayBeforeNextStarvationCheck) {
		nextStarvationCheck = scheduler.getClock().getTime() + delayBeforeNextStarvationCheck;
	}
	...
	public Resource getResourceUsage() {
		...
		synchronized (preemptionVariablesLock) {
			return containersToPreempt.isEmpty() ? getCurrentConsumption()
	 ...
	 * 
	 * @author "yxd,yuxiaoodng"
	 * @param node
	 * @return
	 */
	public Resource assignContainerWithBalancer(FSSchedulerNode node) {
		if (isOverAMShareLimit()) {
			List<ResourceRequest> ask = appSchedulingInfo.getAllResourceRequests();
			Resource amResourceRequest = Resources.none();
			if (!ask.isEmpty()) {
